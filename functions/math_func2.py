...
def realign(im,Nseries,N,M,show_shift=False,return_shift=False,ref_ind=0):
    x=np.zeros(Nseries)
    y=np.zeros(Nseries)
    print("Realigning images...")
    for i in tqdm(range(Nseries)):
        AD=sf.AD(im[:,:,ref_ind],im[:,:,i],N,M)
        ...
        print('Shift focused-defocused:',x[i],y[i])
        fig,axs=plt.subplots(1,3)
        axs[0].imshow(im[:,:,ref_ind])
        axs[1].imshow(im[:,:,i])
        axs[2].imshow(AD)
        plt.show()
        plt.close()
